...
export default function PixelCard({ variant = 'default', gap, speed, colors, noFocus, className = '', children }: PixelCardProps) {
  ...
  const initPixels = () => {
    ...
    console.log('PixelCard init:', { width, height, gap: finalGap, speed: finalSpeed });
    ...
    // Calculate max distance (corner to center) for reversing animation
    ...
    for (let x = 0; x < width; x += parseInt(finalGap.toString(), 10)) {
      for (let y = 0; y < height; y += parseInt(finalGap.toString(), 10)) {
        ...
        // Reverse: pixels at edge start first, center last
        const delay = reducedMotion ? 0 : maxDistance - distance;

        pxs.push(new Pixel(canvasRef.current, ctx, x, y, color, getEffectiveSpeed(finalSpeed, reducedMotion), delay));
      }
    }
    pixelsRef.current = pxs;
    console.log('PixelCard pixels created:', pxs.length);
  };
  ...
  const onMouseEnter = () => {
    console.log('PixelCard hover detected, pixels count:', pixelsRef.current.length);
